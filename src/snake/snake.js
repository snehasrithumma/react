...
export default function Snake() {
    ...
    state.snakePosition.forEach((segment, index) => {
      if (index === 0) {
        if (
          ...
          segment.x === state.rockPosition.x &&
          segment.y === state.rockPosition.y
        ) {
          setState((prevState) => ({
            ...prevState,
            isGameOver: true,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (
        head.x === state.applePosition.x &&
        head.y === state.applePosition.y
      ) {
        //SNAKE EATS APPLE
